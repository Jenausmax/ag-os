...
**Hot-reload:** после успешного INSERT CLI автоматически шлёт `SIGUSR1` живому боту (Unix) или трогает файл-флаг `.ag-os-reload` (Windows) — бот перечитывает таблицу и регистрирует новую задачу в APScheduler без рестарта. Если PID-файл отсутствует или процесс мёртв — warning в stderr, задача остаётся в БД до следующего старта. Подробнее — в [quick-start.md](quick-start.md) раздел 10.5.
