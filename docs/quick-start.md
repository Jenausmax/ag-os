...
Планировщик читает таблицу `schedule` только **на старте**. Если добавить/изменить запись через SQL или CLI во внешнем процессе, живой бот её не подхватит до рестарта. Это решается в AGOS-0029 через hot-reload по `SIGUSR1` — пока же: либо добавляй задачи через Telegram-команды (они идут через живой `AgScheduler.add_task` и регистрируются сразу), либо рестартуй бот.
