...
| После рестарта AG-OS не поднимается агент который создавался через CLI | До AGOS-0037 bootstrap знал только про агентов из `config.agents.permanent`. После 0037 `bootstrap` делает финальный sweep по всем записям в БД и пересоздаёт runtime (tmux-окно или docker-контейнер) для тех, чей артефакт не пережил рестарт. Автоматом. | Обнови до AGOS-0037+ |
